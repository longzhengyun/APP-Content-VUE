<template>
    <div class="product-con" :data-id="data.id" :data-campaignurl="data.campaign_url">
        ...
        <ul class="describe-list">
            <li class="list-item" v-for="(item, key) in data.feature_label" :key="key">{{item}}</li>
        </ul>
...
<style scoped>
    ...
    .product-con .con-title{min-height:.45rem;}
    .product-con .describe-list{display:flex;justify-content:center;min-height:.35rem;font-size:.24rem;color:#999;margin-top:.1rem;}
    .product-con .describe-list .list-item{position:relative;padding:0 .3rem;}
    .product-con .describe-list .list-item::before{content: '·';position: absolute;width: .1rem;height: .1rem;border-radius: 50%;background-color: #ccc;top: 50%;left: .07rem;transform: translateY(-50%);color:transparent;}
